...
### Implementiert
* **Händler-Registrierung:** Formular mit Validierung, Passwort-Hashing, Status `pending` bei Neuanmeldung.
* **Händler-Login/Logout:** Session-basiert, mit Statusprüfung (`pending`/`active`/`suspended`).
* **Admin-Login/Logout:** Separater Session-Bereich für Administratoren.
* **Admin-Dashboard:** Übersicht aller Händler-Konten mit Ein-Klick-Freischaltung/Sperrung.

### Geplant
* **Gast-Zugang:** Eingeschränkte Sicht für nicht angemeldete Besucher.
...
* **Authentifizierung:** Session-basiert über PHP.
  * Händler: `$isLoggedIn = isset($_SESSION['b2b_user_id'])`
  * Admin: `isset($_SESSION['admin_id'])`, geschützt über `includes/admin_auth.php`
  * Wichtig: Die Händler-Session wird zentral in `index.php` gesetzt/geprüft.
  * Das Rendering erfolgt modular in `header.php` / `footer.php` via `require_once __DIR__ . '/...'`.
* **CSS-Styling:** Einheitlicher CSS-Klassen-Präfix `b2b-*` (z. B. `b2b-header`, `b2b-hero`, `b2b-table`); Admin-Bereich nutzt `admin-*`.
...
* **Tabellen:** `haendler` (Geschäftskunden), `admins` (Administratoren)
